...
namespace Eagle._Functions
{
    ...
    internal sealed class Bool : Core
    {
        public Bool(
                    ...
                    if (arguments.Count == (this.Arguments + 1))
                    {
                        ...
                        if (code == ReturnCode.Ok)
                        {
                            try
                            {
                                ...
                                {
                                    value = ConversionOps.ToBool((DateTime)variant1.Value); /* LOSSY */
                                }
                                else if (variant1.IsDouble())
                                {
                                    value = ConversionOps.ToBool((double)variant1.Value); /* LOSSY */
                                }
                                else if (variant1.IsDecimal())
                                {
                                    value = ConversionOps.ToBool((decimal)variant1.Value); /* LOSSY */
                                }
                                else if (variant1.IsWideInteger())
                                {
                                    value = ConversionOps.ToBool((long)variant1.Value); /* LOSSY */
                                }
                                else if (variant1.IsInteger())
                                {
                                    value = ConversionOps.ToBool((int)variant1.Value); /* LOSSY */
                                }
