...
/* ---------------------------------------------------------------------
    ...
    *(sourceEnd - 1) is the last item.
    ...
    Based on: e73b9dc6ccc6adf58ff0bcb941cc8cf56f746fd8 "ConvertUTF.h"
...
/* This library assumes (and uses bit arithmetic for) 32-bit code points. */

COMPILE_TIME_ASSERT(utf32_32bits_size_check, sizeof(UTF32) == 4);
COMPILE_TIME_ASSERT(utf16_16bits_size_check, sizeof(UTF16) == 2);
COMPILE_TIME_ASSERT(utf8_8bits_size_check,   sizeof(UTF8)  == 1);

/* Some fundamental constants */

#define UNI_UTF8_MAX_BYTES		(6)
...
typedef enum {
    conversionOK = 0, 			/* successful, always check vs this */
    sourceExhausted = 1,		/* partial source char, but hit end */
    targetExhausted = 2,		/* not enough room left in target */
    sourceIllegal = 3			/* illegal/malformed source sequence */
} ConversionResult;

typedef enum {
    strictConversion = 0,		/* fail immediately on any error */
    lenientConversion = 1		/* attempt to keep going if possible */
} ConversionFlags;

/* This extern "C" magic is for C++ and does no harm in C */
...
ConversionResult	ConvertUTF32toUTF16(const UTF32 **sourceStart,
			    ...
			    UTF16 *targetEnd, ConversionFlags flags);
UTFXBOOL		isLegalUTF8Sequence(const UTF8 *source,
